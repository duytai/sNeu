...
static void __afl_map_shm(void) {
  ...
  if (id_str) {
    ...
    if (__afl_area_ptr[MAP_SIZE] == 255) {
...
void __sanitizer_cov_trace_switch(u64 Val, u64 *Cases) {
  u64 num_cases = Cases[0];
  u64 *Vals = Cases + 2;

  for (u32 i = 0; i < num_cases; i ++) {
    u8 abs_loss = Val == Vals[i] ? 0 : 64 - __builtin_clzll(Val ^ Vals[i]);
    if ((__afl_loss_ptr[GET_IDX() + i] & abs_loss) || !abs_loss) {
      __afl_loss_ptr[GET_IDX() + i] &= abs_loss;
    }
  }
}
